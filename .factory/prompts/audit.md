...
Receipt minting is coordinator-bounded: the audit coordinator exports the protected launch binding (`FACTORY_CAMPAIGN_AUDIT_ROUND`, `FACTORY_CAMPAIGN_AUDIT_BASE`, `FACTORY_CAMPAIGN_AUDIT_NONCE`) so a bare model call to `machine-receipt.py` fails. Every receipt records `evidence_commit` equal to the campaign audit base and the coordinator round/nonce binding; stale receipts and receipts reused across rounds are rejected by `scripts/check-audit-receipts.py`. Each objective receipt category is authorized by `.factory/campaign-receipt-policy.json`, which binds the category to an allowlisted argv and evidence tier: a receipt covers a category only when its recorded argv equals the allowlisted argv exactly — arbitrary commands (including a bare `true`), tag/path spoofing, and stale round receipts are rejected.

Each `- Executable evidence:` line must carry PASS, FAIL, or BLOCKED and, for PASS/FAIL, a `[receipt: <path>]` (or `[manifest: <path>]` for an accepted runner receipt). Prose cannot certify runtime. Any BLOCKED evidence forces `result: findings`; a report with BLOCKED content may never claim `result: pass`. Every round is bound to one falsification objective from `.factory/campaign-objectives.json`: the round cannot pass without machine receipts (or accepted runner manifests) covering that objective's receipt categories, enforced by `scripts/check-campaign-objectives.py`; replaying the same generic suite cannot satisfy all rounds. Unavailable evidence that blocks conformance must be traceable to an open fact in `.factory/artifacts/blocked-facts.json`, and no conformance completion claim is valid while any fact stays open.

Verify claimed environment evidence using only declared and mechanically evidenced capabilities. Missing backend, GPU, compositor, physical or kernel-backed controller, target consumer, package installation, systemd session, or hardware runner is not a skip and must not be inferred from synthetic evidence. Every conformance claim must be backed by the machine-readable `.factory/artifacts/conformance.json` sidecar: classification, evidence tier, required capabilities, exact evidence commit, and receipt/artifact refs. A `verified` normative real-system/visual/hardware row is a finding when satisfied by a lower evidence tier or by an undeclared/unevidenced capability; a missing contract, probe, receipt, or a skipped probe is unevidenced and is never auto-reclassified. Human-tier claims and golden approval are out-of-band and non-automatable: never accept agent-authored `human: true`, reviewer strings, or environment reviewer identity — force those cases to findings. `required_tier` is assigned by the separate `.factory/requirement-policy.json`, never self-declared by the sidecar. Runner manifests certify nothing until a signer is provisioned in `.factory/signer-trust.json` and the manifest carries a valid signature.
