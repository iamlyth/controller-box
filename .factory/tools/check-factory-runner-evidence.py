...
def expected_probe_authority_digest(commit: str, runner: str) -> str:
    ...
    required=["systemd-run","systemctl","xdg-dbus-proxy","git","bash","python3","ssh-keygen","sudo","busctl","mount","umount","udevadm","stdbuf","dpkg-query","inputplumber-mediator","InputPlumber"]
...
def validate_record(declared: dict, record: dict, commit: str, tree: str,
    ...
    # Independently parse acceptance-critical raw bytes locally. Candidate
    # verdict booleans/markers are never sufficient; this analyzer is loaded
    ...
    for capability in record["capabilities"]:
        ...
        result = subprocess.run(
            [PYTHON, str(analyzer), "--capability", capability,
             "--artifacts", str(artifact_root), "--commit", commit,
             "--tree", tree], cwd="/", capture_output=True, timeout=300,
        )
        if result.returncode != 0:
    ...
    if dbus_desc is not None:
        ...
        result=subprocess.run([PYTHON,str(ROOT/'.factory/runner/inputplumber-dbus-audit.py'),'--audit',str(artifact_root/matches['root-dbus-audit.json']),'--before',str(artifact_root/matches['root-dbus-before.json']),'--after',str(artifact_root/matches['root-dbus-after.json']),'--contract',str(artifact_root/matches['root-dbus-contract.json'])],cwd='/',capture_output=True,timeout=120)
...
def validate(expected_commit: str | None = None, *, expected_campaign_id: str | None = None,
    ...
    with tempfile.NamedTemporaryFile("w", encoding="utf-8", suffix=".toml") as environment_file:
        ...
        if subprocess.run(
            [sys.executable, str(ROOT / ".factory/tools/check-factory-environment.py"), environment_file.name],
