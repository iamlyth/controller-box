#!/usr/bin/env bash
# check-installed-harness-evidence.sh — generic installed-functional evidence gate.
#
...
#
# Usage:
#   .factory/tools/check-installed-harness-evidence.sh [--namespace PATH]
...
if [[ -n "${FACTORY_VERIFIER_ROOT:-}" ]]; then
    ...
    SCRIPT_DIR="$PROJECT_ROOT/.factory/tools"
else
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --namespace)
            if [[ $# -lt 2 || -z "$2" ]]; then
                echo "check-installed-harness-evidence: --namespace requires a non-empty path argument" >&2
                exit 2
            fi
            namespace=$2
            shift 2
            ;;
        *)
            echo "check-installed-harness-evidence: usage: [--namespace PATH]" >&2
            exit 2
            ;;
    esac
done
...
[[ -n "$PINNED_GIT" && -x "$PINNED_GIT" ]] || {
    echo "check-installed-harness-evidence: cannot resolve the pinned Git executable" >&2
    exit 1
}
cd -- "$PROJECT_ROOT"
TOPLEVEL=$("$PINNED_GIT" rev-parse --show-toplevel) || {
    echo "check-installed-harness-evidence: cannot resolve the repository root" >&2
    exit 1
}
[[ "$TOPLEVEL" == "$PROJECT_ROOT" ]] || {
    echo "check-installed-harness-evidence: the canonical root is not the git top level" >&2
    exit 1
}
FINAL_HEAD=$("$PINNED_GIT" rev-parse HEAD) || {
    echo "check-installed-harness-evidence: cannot resolve HEAD" >&2
    exit 1
}
[[ "$FINAL_HEAD" =~ ^[0-9a-f]{40}$ ]] || {
    echo "check-installed-harness-evidence: cannot resolve an exact 40-hex HEAD" >&2
    exit 1
}
...
if [[ -n "$namespace" ]]; then
    ...
    case "$namespace" in
        /*)
            [[ "$namespace" == "$PROJECT_ROOT/"* ]] || {
                echo "check-installed-harness-evidence: the explicit namespace must live inside the repository root: $namespace" >&2
                exit 1
            }
            ns_abs=$namespace
            ;;
    ...
    for ns_part in "${NS_PARTS[@]}"; do
        [[ -n "$ns_part" && "$ns_part" != "." && "$ns_part" != ".." ]] || {
            echo "check-installed-harness-evidence: unsafe namespace path component ('$ns_part')" >&2
            exit 1
        }
    done
    ...
    for rel_part in "${REL_PARTS[@]}"; do
        ...
        if [[ -L "$cursor" || ! -d "$cursor" ]]; then
            echo "check-installed-harness-evidence: namespace parent is a symlink or not a real directory: $cursor" >&2
            exit 1
        fi
    done
    ...
    case "$resolved_abs" in
        ...
        *)
            echo "check-installed-harness-evidence: the namespace escapes the repository root: $ns_abs" >&2
            exit 1
            ;;
    esac
else
    ...
    for rel_part in .factory-state generic-evidence; do
        cursor=$cursor/$rel_part
        if [[ -L "$cursor" || ! -d "$cursor" ]]; then
            echo "check-installed-harness-evidence: the generic evidence root is missing or unsafe ($cursor); run the trusted generic evidence publisher at the exact clean HEAD" >&2
            exit 1
        fi
    done
fi
