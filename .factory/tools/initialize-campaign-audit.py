...
Coordinator sequencing: an existing coordinator is **never overwritten**.  It
is accepted (reused) only when its hardened state is valid and its round and
base_commit exactly match the requested round/base — the active audit's nonce
is then preserved.  Any other pre-existing coordinator (a different round or
base) fails closed as a mismatch, so a second initialization can never
silently re-bind or re-nonce an active audit.

Every trusted Git read of the coordinator uses a pinned absolute immutable
executable (never a PATH-derived ``git``) with a sanitized environment and a
finite timeout, so a poisoned PATH or GIT_* override can never redirect the
binding reads.
"""
...
def coordinator_state(round_number: int, base: str) -> dict | str:
    """The audit coordinator state for this round/base — or the exact match.

    Coordinator sequencing: an existing coordinator is **never overwritten**.
    It is accepted (reused) only when its hardened state is valid and its
    round and base_commit exactly match the requested round/base — the active
    audit's nonce is then preserved.  Any other pre-existing coordinator (a
    different round or base) fails closed as a mismatch, so a second
    initialization can never silently re-bind or re-nonce an active audit.
    """
...
def main() -> int:
    ...
    # fails closed before the report is touched.
...
"""
    ...
    if isinstance(coordinator, str):
        ...
        # The existing exact-matching coordinator was reused; the report is
        # (re)written atomically, the coordinator is never touched again (no
        # overwrite of an active audit's nonce).
