...
def routing(root):
 ...
 if raw_owner!=provenance['unique_owner'] or type(raw_pid) is not int or raw_pid!=provenance['pid']:
  die('raw owner/PID replies differ from root-held provenance')
 ...
 if (source is None or source not in phases['om-before.json']
     or source.rsplit('/',1)[-1]!=pathlib.PurePosixPath(physical_sysfs.group(1)).name):
  die('raw USB/sysfs identity is not bound to the ObjectManager source row')
...
def gpu(root,commit,tree,capability):
 ...
 try:om=decode_object_manager(json.loads(read(cap/'device-type-om.json')),require_nonempty=True)
