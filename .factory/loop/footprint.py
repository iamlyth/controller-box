...
* **special inodes and mount crossings**: the on-disk walk rejects FIFOs,
  sockets, and device nodes beneath the hidden namespaces, and rejects any
  entry whose ``st_dev`` differs from the repository root's (a namespace
  crossing a mount point); the legacy ``.ralph/`` namespace root is
  lstat'ed only (never enumerated, opened, or read) and must be a real
  directory that does not cross a mount point;
...
# modifies entries under ``.ralph/``; after the Ralph control plane was
# removed, *any* tracked ``.ralph/**`` pathname is a migration regression.
# The on-disk ``.ralph/`` directory is foreign recovery history: the
# inventory lstat's the namespace root only and never enumerates, opens, or
# reads anything under it.
...
def _check_tracked(root: Path) -> List[str]:
    ...
    for relpath in sorted(tracked_files(root)):
        ...
        if first in HIDDEN_NAMESPACE_SET:
            ...
            continue
        if first in LEGACY_NAMESPACES:
            # Post-removal: every tracked ``.ralph/**`` pathname is a
            # migration regression (the Ralph control plane was removed, so
            # no legacy file may be tracked at all).
            errors.append(
                f"legacy namespace is tracked (must be removed): {relpath!r}"
            )
            continue
...
def _check_on_disk(root: Path, *, include_runtime: bool = True) -> List[str]:
    ...
    # Legacy ``.ralph/`` recovery history: the namespace root is lstat'ed
    # only — it must be a real directory (never a symlink) and must not
    # cross a mount point.  The foreign directory is never enumerated,
    # opened, or read: its contents are the foreign directory's own
    # business, and the new path must never create or extend it.
    if os.path.lexists(root / ".ralph"):
        ...
        if stat.S_ISLNK(entry.st_mode):
            ...
            errors.append("legacy namespace is not a directory: '.ralph'")
        elif entry.st_dev != root_dev:
