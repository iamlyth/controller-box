...
non-complete task references, matrices that miss or exceed the committed
...
# The separately committed requirement-policy map
# (``.factory/requirement-policy.json``, schema ``ralph-requirement-policy/v1``)
# is the authority for the normative requirement IDs and their required
# evidence tiers (PLAN-01, Task 18 item 3; never self-declared by a plan or
# sidecar). The parser loads it deterministically at parse time and fails
# closed when it is missing, malformed, or divergent. The committed flat
# registry (``.factory/schemas/factory-plan-v1.requirements.json``) mirrors
# the policy's ID set for tooling that consumes a flat ID list; the parser
# verifies the two agree and fails closed on divergence.
REQUIREMENT_POLICY_PATH = (
    Path(__file__).resolve().parent.parent / "requirement-policy.json"
)
REQUIREMENTS_REGISTRY = (
    Path(__file__).resolve().parent.parent
    / "schemas" / "factory-plan-v1.requirements.json"
)
# Populated from the committed policy at import time (never hardcoded): the
# exact Controller requirement-ID set the policy assigns, in policy order.
STABLE_REQUIREMENT_IDS: Tuple[str, ...] = ()
...
def _reject_duplicate_keys(pairs):
    ...
    A duplicate key in any schema/config file (the requirement-policy map
    and the flat registry included)
    ...
    for key, value in pairs:
        if key in result:
            raise PlanError(f"duplicate JSON object key in the requirement-policy registry: {key!r}")
        result[key] = value
    return result
...
_REGISTRY_CACHE: Optional[Tuple[List[str], Dict[str, str]]] = None


def _load_requirement_registry() -> Tuple[List[str], Dict[str, str]]:
    """Load and validate the committed requirement-policy map.

    Returns ``(ids, tiers)`` where ``ids`` is the canonical ordered
    requirement-ID list and ``tiers`` maps every ID to its required evidence
    tier. The policy map is part of the acceptance boundary (Task 18 item 3):
    a missing, malformed, or divergent policy fails closed so free-form
    matrices can never evade the stable ID set. The committed flat registry
    (``factory-plan-v1.requirements.json``) must agree with the policy ID set
    exactly; a drift fails closed instead of silently widening the boundary.
    """
    ...
    try:
        data = json.loads(
            REQUIREMENT_POLICY_PATH.read_text(encoding="utf-8"),
            object_pairs_hook=_reject_duplicate_keys,
        )
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise PlanError(
            f"cannot load the committed requirement-policy map "
            f"{REQUIREMENT_POLICY_PATH}: {exc}"
        ) from exc
    if not isinstance(data, dict) or data.get("schema") != "ralph-requirement-policy/v1":
        raise PlanError(
            "the requirement-policy map must be an object with schema "
            "`ralph-requirement-policy/v1`"
        )
    entries = data.get("requirements")
    if not isinstance(entries, list):
        raise PlanError(
            "the requirement-policy map must declare a `requirements` array"
        )
    ids: List[str] = []
    tiers: Dict[str, str] = {}
    seen: set = set()
    for entry in entries:
        if not isinstance(entry, dict):
            raise PlanError("every requirement-policy entry must be an object")
        rid = entry.get("id")
        tier = entry.get("required_tier")
        if not isinstance(rid, str) or not MATRIX_ID_RE.fullmatch(rid):
            raise PlanError(
                f"the requirement-policy map has an invalid ID: {rid!r}"
            )
        if rid in seen:
            raise PlanError(
                f"the requirement-policy map has a duplicate ID: {rid}"
            )
        if not isinstance(tier, str) or not tier:
            raise PlanError(
                f"the requirement-policy map has an invalid required tier "
                f"for {rid}: {tier!r}"
            )
        seen.add(rid)
        ids.append(rid)
        tiers[rid] = tier
    if not ids:
        raise PlanError("the requirement-policy map declares no requirements")
    try:
        registry = json.loads(
        ...
        raise PlanError(
            f"cannot load the committed requirement registry "
            f"{REQUIREMENTS_REGISTRY}: {exc}"
        ) from exc
    if not isinstance(registry, dict) or not isinstance(
        registry.get("requirement_ids"), list
    ):
        raise PlanError(
            "the requirement registry must be an object with a "
            "`requirement_ids` array"
        )
    registry_ids = registry["requirement_ids"]
    if list(registry_ids) != ids:
        raise PlanError(
            "the requirement registry must contain exactly the "
            "requirement-policy IDs, in policy order"
        )
    _REGISTRY_CACHE = (list(ids), dict(tiers))
    return _REGISTRY_CACHE


# The stable requirement-ID set and the required-tier map are derived from the
# committed policy at import time (never hardcoded): the parser, the hidden
# suite, and the fixture plan tool share the exact Controller ID set the
# policy assigns, in policy order.
STABLE_REQUIREMENT_IDS = tuple(_load_requirement_registry()[0])
REQUIREMENT_TIERS: Dict[str, str] = dict(_load_requirement_registry()[1])
...
def _parse_matrix(block: Block, tasks: List[Task], lifecycle_status: str) -> List[MatrixRow]:
    ...
    for line in table_lines[2:]:
        ...
        if classification == "verified":
            # A migrated product plan can contain baseline evidence that
            # predates its current task ledger, so an empty task-reference
            # cell is valid. When references are present they remain strictly
            # bound to completed tasks. Classification authority is the
            # separately validated conformance sidecar, not this prose table.
            non_complete = sorted(
    ...
    # Active product plans may retain previously verified rows. The parser
    # requires those rows to reference completed tasks; the separately
    # confined conformance validator binds classifications to the committed
    # sidecar and evidence, preventing a planner from elevating them here.
    registry, _tiers = _load_requirement_registry()
    extra_ids = sorted(rid for rid in seen_ids if rid not in registry)
    if extra_ids:
        raise PlanError(
            "conformance matrix has a requirement ID outside the committed "
            "requirement-policy registry: "
            f"{', '.join(extra_ids)}"
        )
    ...
    if missing_ids:
        raise PlanError(
            "conformance matrix must cover every ID in the committed "
            "requirement-policy registry "
