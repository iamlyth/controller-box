...
| PKG-03 | §9.1, §9.3 | verified | `service_install.c` implements `cbx_service_install()` with atomic write, systemctl enable. `cbx_manager_check_first_run()` in `manager.c` detects first-run (no `controller-box.service` in `~/.config/systemd/user/`) and shows modal "Enable overlay service?" dialog. Controller A confirms → `cbx_service_install()` → writes unit file → `systemctl --user enable --now`. Controller B cancels. Pointer path: mouse click on Yes/No buttons. Both paths tested through `cbx_manager_handle_event` / `cbx_manager_handle_mouse_event` (production dispatch) in `test_manager_interaction_ctrl.c`. Existing direct-call tests in `test_service_install.c` remain as supplemental. | Task 7 |
...
**First-run service install (M39):** Spec §9.1 first-run "Enable overlay service?" dialog in the manager. `cbx_manager_check_first_run()` in `manager.c` detects missing `controller-box.service` and shows modal dialog. Controller A confirms → `cbx_service_install()` writes unit file + `systemctl --user enable --now`. Controller B cancels. Pointer path: mouse click on Yes/No buttons. Both paths tested through `cbx_manager_handle_event` / `cbx_manager_handle_mouse_event` (production dispatch) in `test_manager_interaction_ctrl.c`: `test_first_run_confirm_controller_path`, `test_first_run_cancel_controller_path`, `test_first_run_confirm_pointer_path`, `test_first_run_cancel_pointer_path`.
...
- Status: complete
- Dependencies: none
- Scope: `src/manager/manager.c` (first-run detection + dialog), `src/manager/manager.h` (struct fields + API), `src/manager/service_install.c` (verify called from UI), `tests/test_manager_interaction_ctrl.c` (production-dispatch test), `tests/test_installed_smoke.sh` + `tests/test_installed_binary.sh` (pre-create service file to suppress dialog)
...
- Verification: `nix-shell --run "ctest --test-dir build-check -R 'service_install|interaction_ctrl' --output-on-failure"` — both pass (0.11s + 0.66s). 4 new tests: `test_first_run_confirm_controller_path`, `test_first_run_cancel_controller_path`, `test_first_run_confirm_pointer_path`, `test_first_run_cancel_pointer_path`. Each dispatches through `cbx_manager_handle_event` (production path) with mocked systemctl. Full suite 98/98 pass (2 hardware skips).
