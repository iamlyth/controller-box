...
Runner: none
Evidence: Retained the prior host-mode implementation after evaluating the recorded audit labels against current source. The exclusive host selects any row (Up/Down) and edits slot (Left/Right) and profile (L1/R1) through production dispatch (DBus InputEvent via cbx_overlay_input_cb and SDL keyboard via cbx_overlay_service_step), documented in README.md and host_mode.h; non-host mutation is rejected by cbx_host_mode_handle's row_idx!=host_row freeze guard before any grid write (test_overlay_interaction test_o11e/test_o11f, test_overlay_native test_o06b). Entry/selected-row/exit transitions dirty the pre-built surface (cbx_overlay_on_host_mode_change marks dirty once per real transition; row navigation dirties at dispatch; slot/profile reuse cbx_overlay_on_slot_change / cbx_overlay_on_profile_change), and test_overlay_visual proves pixel-level HOST/SELECTED/FROZEN region differences and profile-text changes. The functional, spec-compliance and lint re-audits reported no BLOCKER; cbx_host_mode_reconcile (called from cbx_overlay_reconcile_hotplug and overlay_backend_ready) and cbx_overlay_on_lifecycle_closed already close the hotplug/host-disconnect/close-lifetime gaps those audits raised. The efficiency audit's single BLOCKER was genuine and is now closed: cbx_overlay_service_step ticked every armed poll on each 50 ms timer event (O(C^2) synchronous reads starving the render loop), and now ticks only the poll named by event.user.data1, matched against svc->polls[0..CBX_MAX_COMPOSITES). The poll test harnesses in test_overlay_interaction/test_overlay_native/test_overlay_reconcile/test_installed_functional now carry the owning poll exactly as ip_intercept_poll's SDL timer callback does, and the new test_overlay_service test_step_ticks_only_owning_poll drives the real step loop with two polls: with the legacy fan-out it fails (non-owner error_count 1), with the fix it passes (owner PASS_WAIT->ACTIVE, non-owner error_count 0). Verification on develop: ctest --test-dir build -R 'test_host_mode|test_overlay_interaction|test_overlay_native|test_overlay_visual|test_golden' --output-on-failure -> 5/5 passed; ./scripts/verify.sh -> exit 0, 91/91 passed, 0 failed, 172.04 s, only the two documented exit-77 hardware skips (test_kernel_controller /dev/uinput; test_backend_smoke no video device, test_backend_smoke_sw passes); ./scripts/verify-sanitizers.sh -> exit 0, 91/91 passed, no ASan/UBSan defects. Logs: .factory/artifacts/logs/task4-verify.log sha256 99a3e9186c46d2641a6ca3d5e501ef1cdcc716d92058acf127070b3412beeba0; .factory/artifacts/logs/task4-verify-sanitizers.log sha256 0b87c79695e601f7c14747f6af109181258ca62b667a1be1848e7492417a6f95. Full ownership/generation and stale-queued-event hygiene remains task 10's scope; this task closes the fan-out defect that directly starved its transition rendering.
