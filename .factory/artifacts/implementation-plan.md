...
Runner: none
Evidence: repair round: manager readiness retry no longer gates on the stale dbus_connected flag and every failed required step fails closed via cbx_manager_backend_degraded (owner verification, enumeration, PropertiesChanged subscription) in src/manager/manager.c. Added native regressions test_manager_transient_recovery_retries_without_owner_loss and test_manager_recovery_exhaustion_fails_closed in tests/test_manager_native.c; the former fails against the pre-fix code with -EAGAIN (-11) and passes after the fix. ./scripts/verify.sh exit 0 (91/91 ctest, 2 hardware skips); targeted ctest regex exit 0 (7/7); ./scripts/verify-sanitizers.sh exit 0 (91/91, no ASan/UBSan defects).
