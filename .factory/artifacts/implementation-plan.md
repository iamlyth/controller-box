...
- Source: Task 32 project-gate evidence: `test_installed_functional_evidence` still reports an escaping or otherwise invalid `FACTORY_INSTALLED_FUNCTIONAL_EVIDENCE_PATH` override. Current inspection confirms the override boundary is implemented in both `scripts/verify-project.sh` and `scripts/check-installed-functional-evidence.sh`, but the pending task must verify the complete contract rather than infer acceptance from lexical path checks: canonical ancestor safety, target ownership/mode/link-count, no-replace publication, and exact receipt binding remain required. This is a factory/project verification finding, not evidence that product acceptance passed.
...
- Acceptance criteria:
  ...
  - A regression reproduces the reported escaping override failure and passes after the fix; tests also prove symlinked ancestors, hardlinks, wrong owner/mode, pre-existing unsafe destinations, and replacement races fail closed without overwriting or writing evidence; no test weakens the rejection or blesses skipped/early-exit installed acceptance.
