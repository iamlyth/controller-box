...
Runner: none
Evidence: W1/W2 implemented on the production paths. W1 — host-mode entry/exit dirties the pre-built surface: cbx_overlay_on_host_mode_change (src/app/overlay_service.c) is wired as hm.on_state_change in run_overlay_service and fires mark_dirty_all on every real enter/exit transition, and cbx_host_mode_enter/exit fire it through cbx_host_mode_toggle and the CBX_HM_R3 handle path. The on-entry frame is proven materially different from Player Mode via fb_frames_differ in test_overlay_visual, and the enter/exit dirty triggers are asserted through the production DBus dispatch path (cbx_overlay_input_cb) in test_o13/test_o13b. W2 — dirty-trigger policy reconciled to SPEC §4.9: dirty fires on device change (cbx_overlay_reconcile_hotplug), slot change (cbx_overlay_on_slot_change), profile change (cbx_overlay_on_profile_change), and host-mode state transitions (cbx_overlay_on_host_mode_change); the show/save/close mark_dirty_all calls were reviewed and are deliberate (lifecycle show_surface/begin_fade_in re-render on show, cbx_overlay_on_save re-renders after a committed save, close re-renders via the close-path save callback). The R3-exit dirty trigger now fires exactly once: neither the DBus (cbx_overlay_input_cb) nor the SDL (cbx_overlay_service_step) dispatch re-exits host mode on CBX_HM_RESULT_EXIT (R3 is handled inside cbx_host_mode_handle), and cbx_host_mode_exit now fires the transition only on a real active→inactive state change (no spurious no-op dirty trigger). New regression tests: tests/test_host_mode.c test_exit_noop_no_transition; tests/test_overlay_interaction.c test_o13c_host_transition_fires_once (single-fire enter+exit through production dispatch + no-op redundant exit). Verification run: nix-shell --run 'cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug && cmake --build build --parallel' OK; ctest --test-dir build -R 'test_overlay_visual|test_overlay_interaction|test_golden' --output-on-failure -> 100% passed (3/3); ./scripts/verify.sh -> 100% passed (91/91, 0 failures; only runner-gated skips test_kernel_controller and test_backend_smoke).
