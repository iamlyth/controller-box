...
Runner: none
Evidence: Repaired the InterceptMode poll ownership/lifetime defects in src/dbus/ip_intercept_poll.[ch] and src/app/overlay_service.c. ip_intercept_poll gained an arm generation advanced on every start/stop; sdl_timer_cb carries it in SDL_UserEvent.user.code and cbx_overlay_service_step ticks a poll only when pointer ownership and the live generation both match, so a stale event queued before a stop/rearm/rebuild cannot tick a re-used slot. start() stays IDLE until SDL_AddTimer succeeds and reverts to IDLE on failure; stop() removes the timer and invalidates the generation; poll_error_reset() now removes the timer and invalidates the generation, so an error cannot leak a live timer or a stale event. The ACTIVE watchdog was removed (a legitimate session longer than ten seconds is never a failed close; PASS_WAIT still resets on ~10 s of unexpected NONE). cbx_overlay_rearm_polls() rolls back via overlay_stop_all_polls() when any composite fails to arm, so sparse timer-start success cannot leave an active poll outside cleanup/dispatch bounds; overlay_stop_all_polls() iterates the full CBX_MAX_COMPOSITES array and is used for rearm, overlay_recover() and shutdown. ObjectManager-only device discovery is unchanged. Tests (all real production paths): test_intercept_poll gains an SDL timer fixture and injects an SDL_AddTimer failure through the linker --wrap mechanism (tests/CMakeLists.txt) proving start-failure leaves IDLE with no timer, stop removes the timer and advances the generation, rearm invalidates the previous generation, an error removes the timer, and ACTIVE stays ACTIVE for 10 ticks with max_timeout_ticks=3 then deactivates on PASS. test_overlay_service adds a wrong-generation (stale) event rejection test, a linear per-device read-count test (one 50 ms event per device = one get_property each; three device-0 events add exactly three reads), and an IDLE no-DBus-read test. test_overlay_reconcile adds full rearm (one timer per composite) and partial-start-failure rollback (empty composite path makes poll 1 fail; poll 0's timer is rolled back) assertions, and its push_poll_event now sends the live generation. test_overlay_native, test_overlay_interaction and test_installed_functional event producers were updated to send the live generation. test_manager_calls now replaces unconsumed GamepadOrder expectations with exact zero-write assertions using the new ip_dbus_mock_call_count consumption tracking (tests/dbus_mock.[ch]). A/B proof (temporary revert of the production fix, then restored byte-identical): the old ACTIVE watchdog made test_tick_active_long_session_no_timeout fail, the old start()/poll_error_reset() made test_timer_start_failure_leaves_idle and test_timer_error_removes_timer fail, and the old sparse rearm made test_rearm_partial_start_failure_stops_all_timers fail; all pass again after restore. Verification: ./scripts/verify.sh exit 0 (clean build, 91/91 ctest passed, 0 failed, only the two exit-77 hardware skips test_kernel_controller and test_backend_smoke; packaging/installed all passed), ctest --test-dir build -R 'test_intercept_poll|test_overlay_service|test_overlay_reconcile|test_overlay_latency|test_daemon_footprint|test_manager_calls' --output-on-failure exit 0 (6/6), nix-shell --run './scripts/verify-sanitizers.sh' exit 0 (91/91, no ASan/UBSan defects). Logs (gitignored): .factory/artifacts/logs/task10-verify.log sha256 5e14413cb19ee56c5aa20f0fb1d30008ce1455980b127741626b22bf3afc3d7f, .factory/artifacts/logs/task10-sanitizers.log sha256 a4e0888e07843b80f432e387b6e1415eda90b629af30e3f4a411b950ad527d64.
