...
Runner: none
Evidence: The name-input and delete-confirm modal dialogs in src/manager/profiles_tab.c expose pointer-reachable Confirm/Cancel buttons (dialog_confirm_btn/dialog_cancel_btn) that are shown only while a dialog is active and route through cbx_manager_handle_mouse_event -> cbx_panel/button hit-test -> on_dialog_confirm_pressed/on_dialog_cancel_pressed -> the same production actions as A/B (cbx_profiles_tab_name_input_confirm, cbx_profiles_tab_name_input_cancel, cbx_profiles_tab_confirm_delete, cbx_profiles_tab_cancel_delete). The name-input dialog shows both Confirm and Cancel; the delete-confirm dialog shows both. While a dialog is modal the always-present list / Create / Edit / Delete action buttons are hidden so a stray pointer click cannot abandon or corrupt the dialog. Pointer-path tests test_prof_name_input_confirm_pointer (M15), test_prof_name_input_cancel_pointer (M16), test_prof_delete_confirm_pointer (M19), test_prof_delete_cancel_pointer (M20), plus modal-isolation tests, dispatch real mouse_motion + mouse_button events through cbx_manager_handle_event and assert both hover and the semantic outcome (editor opens / list restored / file unlinked / no file created). The interaction inventory M15/M16/M19/M20 entries use the real dialog_confirm_btn/dialog_cancel_btn path; fixed the stale M16 assertion in test_interaction_inventory.c (originally expecting CBX_PATH_NA / NOT_APPLICABLE for name-input cancel, contradicting the now-present Cancel button) to CBX_PATH_AVAILABLE / VERIFIED, which is what allowed the full gate to pass. Verification run: cmake --build build --parallel OK; focused `ctest --test-dir build -R 'test_manager_interaction_prof|test_profiles_tab|test_interaction_inventory|test_manager_visual' --output-on-failure` -> 100% passed (4/4); full `./scripts/verify.sh` -> 100% passed (91/91, 0 failures; only runner-gated skips test_kernel_controller needing /dev/uinput and test_backend_smoke needing a GPU runner, the software twin test_backend_smoke_sw passing).
