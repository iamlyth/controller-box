...
Runner: none
Evidence: src/dbus/ip_properties.c is now wired into the production overlay backend wiring. Added cbx_overlay_on_prop_change() (production PropertiesChanged callback) and cbx_overlay_props_wire() (production ip_properties_init + ip_properties_subscribe) to src/app/overlay_service.c, plus a cbx_reactive_props field (`props_state`) and ip_properties handler (`props`) in cbx_overlay_service_ctx. cbx_overlay_props_wire() is invoked in BOTH production paths that establish a live connection: run_overlay_service() (main init, after expected_sender is resolved) and overlay_backend_ready() (backend reacquisition/recovery). External changes to GamepadOrder, ProfileName, ProfilePath, TargetDevices and SourceDevicePaths are observed reactively — the callback refreshes the matching props_state entry and marks the surface dirty so a re-render reflects InputPlumber's live property state. An externally-injected PropertiesChanged now updates internal state through the real subscribed handler (no dead code): added tests test_props_wire_subscribes, test_props_wire_no_backend, test_props_inject_gamepadorder_updates_state, test_props_inject_all_slots, test_props_inject_wrong_sender_no_update, and test_props_inject_invalidated_clears to tests/test_overlay_service.c, which inject PropertiesChanged signals via the mock bus and assert svc->props_state is updated (including fail-closed rejection of a spoofed sender and invalidation clearing). Verification run: nix-shell --run 'cmake --build build --parallel' OK; ctest --test-dir build -R 'test_properties_changed|test_overlay_native|test_manager_native|test_native_dbus' --output-on-failure -> 100% passed (6/6); ./scripts/verify.sh -> 100% passed (91/91, 0 failures; only runner-gated skips test_kernel_controller and test_backend_smoke that are environmental).
