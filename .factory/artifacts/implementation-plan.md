...
- Verification: `nix-shell --run 'bash tests/test-visual-audit.sh'` exit 0 (13d/13e/13f/13g pass; new 13h `installed-binary provenance hardening passed (13h)`); the real installed driver captured manager-main to a valid PNG with an exact-commit receipt under a private Xauthority; shellcheck `scripts/visual-capture-driver.sh tests/test-visual-audit.sh` clean; `scripts/check-context-summary.py` passes after regenerating the context summary. No golden regen; no human/visual acceptance claim; no conformance evidence elevation.
