...
Runner: none
Evidence: W1: added the deliberate on_state_change dirty trigger to host mode (src/overlay/host_mode.h/.c: new cbx_hm_state_change_cb fired by cbx_host_mode_enter()/cbx_host_mode_exit(), covering both the direct API and cbx_host_mode_toggle()) and wired it in the production overlay service (src/app/overlay_service.c: new exported cbx_overlay_on_host_mode_change() wired as svc->hm.on_state_change in run_overlay_service step 10), so entering and exiting host mode now mark the pre-built surface dirty through both live dispatch paths and on_host_mode_change is exported in overlay_service.h. Dirty-trigger policy is now uniform with the spec 4.9 set — device change (overlay_backend_ready / cbx_overlay_reconcile_hotplug), slot change, profile change, host-mode state transitions, host navigation (CBX_HM_RESULT_* MOVED/SLOT), and show/save/close (lifecycle show_surface / cbx_overlay_on_save) all mark dirty deliberately rather than ad hoc. Tests: test_host_mode.c adds test_state_change_on_enter_exit + test_state_change_on_toggle (callback fires with active=true on enter and false on exit; MOVED/FROZEN do not fire); test_overlay_interaction.c adds test_o13_host_entry_marks_dirty_dbus + test_o13b_host_exit_marks_dirty_dbus driving the production DBus dispatch (cbx_overlay_input_cb) with the lifecycle inactive so the flag is observed before the active-only re-render consumes it — surface is dirty on host-mode entry and again on exit; test_overlay_visual.c adds test_host_entry_dirty_render_differs proving W1 outcome via the production-equivalent dirty trigger: host-mode entry marks the surface dirty, the re-render is consumed, and the presented Host Mode frame materially differs from the Player Mode frame (visual region diff). Demonstrably non-vacuous: with fire_state_change disabled the new test_host_mode and test_overlay_interaction tests FAIL (2/2 + 2/24), confirmed on a real build. Verification run: nix-shell --run 'cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug && cmake --build build --parallel' OK; ctest --test-dir build -R 'test_overlay_visual|test_overlay_interaction|test_golden|test_host_mode' --output-on-failure -> 100% passed (4/4); task regex ctest --test-dir build -R 'test_overlay_visual|test_overlay_interaction|test_golden' -> 100% passed (3/3); ./scripts/verify.sh -> 100% passed (91/91, 0 failures; only runner-gated skips test_kernel_controller and test_backend_smoke).
