...
Runner: none
Evidence: Repair cycle 1 (security BLOCKER): fixed hotplug host-mode privilege shift / permanent freeze. Added `cbx_host_mode_reconcile` (re-resolves host+selected rows by persistent ID, exits host mode when the host is gone) and grid-aware `cbx_host_mode_enter_with_grid`/`cbx_host_mode_toggle_with_grid` that record the host identity; wired reconcile into `cbx_overlay_reconcile_hotplug` and `overlay_backend_ready`, and production DBus/SDL host entry in `src/app/overlay_service.c` now uses the grid-aware toggle. Reused existing `cbx_overlay_on_host_mode_change` dirty trigger (no double-fire). Commands: `./scripts/verify.sh` → exit 0, 91/91 run passed (2 hardware skips: test_kernel_controller, test_backend_smoke); `ctest --test-dir build -R 'test_host_mode|test_overlay_interaction|test_overlay_native|test_overlay_visual|test_golden' --output-on-failure` → 5/5 passed. New regression tests: `tests/test_host_mode.c` (host row shift, host removed→exit, selected-row removed→host fallback, persistent-ID preferred over reused composite path, empty grid, bounds, toggle-with-grid identity) and `tests/test_overlay_reconcile.c` (production `cbx_overlay_service_step` hotplug with host active: host removed → host mode exits and survivor unfrozen; host row shifts → host privilege follows persistent ID to new index). Repair cycle 2 (functional BLOCKER — Host Mode state leaked across overlay close/reopen): wired `svc->lifecycle.on_closed` in `run_overlay_service` to new production callback `cbx_overlay_on_lifecycle_closed` (`src/app/overlay_service.c`, declared in `src/app/overlay_service.h`) which calls `cbx_host_mode_exit(&svc->hm)`. Every IDLE transition (host B-press close, poll deactivation, visible timeout, backend loss/`force_close`, shutdown) now ends Host Mode, so a reopened overlay starts in Player Mode instead of presenting every non-host controller frozen with only the stale host able to exit. Reused existing `cbx_host_mode_exit` W2 no-op contract and `cbx_overlay_on_host_mode_change` dirty trigger (no double-fire). Added production-path regression `tests/test_overlay_interaction.c::test_host_mode_exits_on_close_and_reopen` whose fixture mirrors the production `on_closed` wiring: enter Host Mode via DBus R3 → assert row 0 SELECTED / row 1 FROZEN → host B-close → IDLE → assert host mode inactive, all rows `CBX_ROW_NORMAL` and unfrozen → re-activate → new session starts Player Mode and the previously-frozen controller 1's R3 makes it host. Also added `tests/test_overlay_native.c::test_host_mode_exits_on_close_native`, which drives the real native DBus InputEvent transport for R3 entry and B-close and then re-activates; the native fixture now mirrors the production `on_closed` wiring. Non-vacuity proven: neutralizing the callback body failed the new interaction test (27/28) before restoring. Audit note: the other two BLOCKER headings carried no actionable defect — 'linting I6' is explicitly a pre-existing long function outside this task (recommendation: do not churn), and every spec-compliance item was independently verified ✔. Commands: `nix-shell --run './scripts/verify.sh'` → exit 0 (91/91 run passed, 2 hardware skips: test_kernel_controller, test_backend_smoke); `nix-shell --run "ctest --test-dir build -R 'test_host_mode|test_overlay_interaction|test_overlay_native|test_overlay_visual|test_golden' --output-on-failure"` → exit 0 (5/5 passed). Repair cycle 3 (audit findings: security degraded-PersistentId identity fallback; spec failed profile apply left grid mutated with no dirty trigger): (1) Host Mode identity is now stability-aware. Added `bool id_stable` to `cbx_grid_composite_info`/`cbx_grid_row` (`src/overlay/grid_render.h`), copied in `cbx_select_grid_build`; `fill_composite_info` (`src/app/overlay_service.c`) sets it true only for a real non-empty InputPlumber PersistentId and marks the index-derived `composite-<index>` fallback degraded. `record_row_identity` (`src/overlay/host_mode.c`) records only the id for a stable row and only the composite path for a degraded row; `resolve_row_identity` treats a recorded stable id as authoritative and never falls back to the (reusable) composite path when it no longer matches, while a degraded path is matched only against another degraded row. This preserves the SPEC §4.4 no-privilege-inheritance invariant across hotplug/backend rebuilds: a different controller that reuses the old host path or now reports a stable id can never inherit host. New `tests/test_host_mode.c` regressions: `test_reconcile_stable_id_reused_path_exits` (old behavior failed open to X on the reused path), `test_reconcile_degraded_id_matches_same_path`, `test_reconcile_degraded_id_stable_row_exits`; `tests/test_overlay_reconcile.c` degraded-id hotplug tests still pass (path re-resolution). (2) `cbx_overlay_on_profile_change` (`src/app/overlay_service.c`) now marks the pre-built surface dirty before any early return so a profile-change event always re-renders (§4.9) even when the engine apply fails, and on `cbx_profile_cycle_apply` failure rolls the displayed profile back to the engine's authoritative `ProfilePath` (reverse-mapped through the loaded profile list) so the grid cannot misrepresent live engine state. New production-path regression `tests/test_overlay_interaction.c::test_o12b_profile_apply_failure_rolls_back_and_dirties` drives DBus InputEvent dispatch with `LoadProfilePath` failing and asserts the displayed profile is restored and the surface is dirty. Commands: `./scripts/verify.sh` → exit 0 (91/91 run passed, 2 hardware skips: test_kernel_controller, test_backend_smoke); `ctest --test-dir build -R 'test_host_mode|test_overlay_interaction|test_overlay_native|test_overlay_visual|test_golden' --output-on-failure` → exit 0 (5/5 passed). Audit-header note: the security section was the BUG-0019 parser false positive (the audit itself recorded 0 BLOCKER / 1 WARN / 3 INFO); the actionable WARN was the degraded-identity fallback fixed here.
