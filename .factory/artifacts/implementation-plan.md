...
- Scope: add a strict exact-commit ordered registry of fixed mandatory pre-round hook implementations; run every enabled hook once before each round planner with durable start/completion cursors; move Ollama usage policy out of per-model `authorize_launch` into the registry and commit that entry disabled while preserving all enabled hooks; digest-bind configuration and typed results in the sole campaign state; preserve redaction, bounded supervision, one writer, installed-copy verification, foreign `.ralph`/`.factory-state` non-access, and finite outcomes.
- Acceptance criteria: ordered enabled hooks run exactly once before each round planner and not on planner retry; disabling/removing Ollama does not suppress the enabled branch guard; ambiguous started recovery never reruns a hook; mandatory failure prevents the planner and terminates finitely; per-model launch exposes no quota/cookie driver surface; configuration/results are exact-commit digest-bound; no raw hook output or credential material enters state or prompts; unit, adversarial, installed, migration, and five-round tests pass.
