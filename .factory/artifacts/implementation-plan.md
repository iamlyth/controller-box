...
Runner: none
Evidence: PropertiesChanged now carries and validates object identity: ip_properties_changed_payload gained object_path + iface_name (src/dbus/dbus_interface.h), populated from sd_bus_message_get_path in sd_properties_changed_callback (src/dbus/dbus_client.c), and the ip_prop_changed_cb dispatch passes both to the consumer. ip_properties_handle_changed (src/dbus/ip_properties.c) rejects a sender, emitting interface, or object-path class that does not match the property (GamepadOrder→Manager@IP_DBUS_PATH/Manager; ProfileName/ProfilePath/TargetDevices/SourceDevicePaths→CompositeDevice@IP_DBUS_PATH/CompositeDeviceN) and issues exactly one bounded authoritative get_property read on IP_PROP_TYPE_INVALIDATED before dispatching the refreshed value (or the invalidation when the read fails). Per-device storage replaced the old process-global props_state: cbx_composite_entry holds profile_name/profile_path/target_devices/source_device_paths and cbx_device_model holds the Manager gamepad_order; cbx_device_model_apply_property updates only the entry named by object_path (unknown paths are rejected but never created), so one device cannot overwrite another. cbx_overlay_on_prop_change applies validated changes to svc->model and refreshes the matching grid row's displayed profile before dirtying the surface; cbx_manager_on_prop_change applies to ct.model and rebuilds the Controllers list from the model (routed-profile label), wired through cbx_overlay_props_wire / cbx_manager_props_wire. Reactive props survive re-enumeration via cbx_device_model_preserve_props (reconcile_enumerate, cbx_controllers_tab_refresh). Tests: test_properties_changed 40/40 (identity propagation, wrong interface/path, malformed path, bounded-read count==1, read-failure clear); test_overlay_native 35/35 including two-composite isolation, rendered grid-profile update, native authoritative invalidation read, and a subscription-replacement test asserting the stale callback is not invoked; test_manager_native 23/23 including per-device isolation and Controllers-list label update; test_native_dbus passes. `./scripts/verify.sh` exit 0, 91/91 CTest (only the two exit-77 hardware skips); `./scripts/verify-sanitizers.sh` exit 0, 91/91 under ASan+UBSan.
