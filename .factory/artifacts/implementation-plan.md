...
Runner: none
Evidence: Resolved BUG-0016 in src/manager/profile_editor_list.c. cbx_profile_editor_refresh (src/manager/profile_editor_list.c) now enumerates the whole 17-button virtual-button catalog via s_catalog_order (NES-minimum order from profile_validate.c, then the remaining cbx_diag_button entries), showing unbound rows as "button → (unbound)" and appending any non-catalog source mapping so no binding is hidden, instead of iterating only profile.mapping_count. New cbx_profile_editor_row_count/row_button/row_mapping accessors expose the row→button/mapping table; sync_diagram_highlight highlights the row's catalog button (bound or unbound). Activating an unbound row calls the shared cbx_profile_editor_find_or_create_mapping (moved from profile_editor_seq.c so list and sequential modes never diverge) and opens BINDING_EDIT without duplicating an existing mapping; begin_target_pick/begin_capture now resolve the edited mapping from editing_index or the selected row. on_binding_selected now syncs the editor selection to the clicked row, so the pointer path activates the exact row rather than whatever was previously selected. Empty profiles still reach sequential capture via A (mapping_count == 0 path retained). Default immutability: refresh never mutates the profile; test_default_profile_immutable_under_catalog loads the shipped data/profiles/default.yaml through cbx_profile_load and asserts mapping_count 6, not-dirty, 17 catalog rows and exactly 11 unbound. Tests added to tests/test_editor_list_mode.c: test_catalog_enumerates_all_seventeen (all 17 buttons), test_default_profile_immutable_under_catalog (zero-mapping/add action), test_activate_unbound_row_creates_mapping and test_activate_existing_row_no_duplicate, test_activate_unbound_row_pointer_path (real list-widget on_select), test_catalog_scrolling, test_diagram_regions_all_buttons (per-button highlight framebuffer region diff at the renderer's content rect). tests/test_manager_native_prof.c adds test_m29b_unbound_row_activation_ctrl and test_m29c_unbound_row_activation_pointer through cbx_manager_handle_event for both production activation paths. Verification: ./scripts/verify.sh exit 0 (91/91 ctest passed, 0 failed, only the two exit-77 hardware skips test_kernel_controller and test_backend_smoke; log .factory/artifacts/logs/task15-verify.log sha256 be6e23a5b2f83305abbde28e577340c86a8ca0ff5afa082e9cfec7e32b8b6f48); ctest --test-dir build -R 'test_editor_list_mode|test_profile_diagram|test_profiles_tab|test_manager_native|test_manager_visual' --output-on-failure exit 0 (6/6 passed; log .factory/artifacts/logs/task15-targeted.log sha256 4f9b76eef1961ee997a0cbb5b7d860638e602ca1cff712cfc4a452d65369ff42).
