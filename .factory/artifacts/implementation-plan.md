...
Runner: none
Evidence: verification exit 0 on local. Repair cycle: BLOCKER on recovery re-subscription (repeated cbx_overlay_props_wire / input-events / hotplug re-wiring after InputPlumber restarts leaked sd-bus slots, daemon match rules, and heap records and duplicated dispatch) fixed at the root by making sd_subscribe_signal idempotent per (interface, member) in dbus_client.c and mirroring that in the mock backend (dbus_mock.c); re-subscribe refreshes the existing binding instead of appending a new one. Added test_props_wire_idempotent_rewire in test_overlay_service.c. Verified: ./scripts/verify.sh exit 0 (91/91, only runner-gated skips) and ctest --test-dir build -R 'test_properties_changed|test_overlay_native|test_manager_native|test_native_dbus' --output-on-failure exit 0 (5/5).
    Repair Cycle 2 (BLOCKER — changed properties unreachable in production): sd_properties_changed_callback misused sd_bus_message_peek_type (src/dbus/dbus_client.c), which writes the element type character into its first out-parameter and returns only a status code (1/0/errno). Passing NULL as the first arg discarded the type char, so `vtype == 'v'` was never true and every changed property in the a{sv} dict took the skip path — only invalidated `as` entries fired, leaving GamepadOrder/ProfileName/ProfilePath/TargetDevices/SourceDevicePaths value changes unobserved (dead production parse path). Fixed the peek call to capture the type char (`char vtype; peek_type(msg, &vtype, &contents_ptr)`); the string- and array-variant dispatch in the a{sv} dict now executes. Added native end-to-end coverage in test_overlay_native.c that emits faithful org.freedesktop.DBus.Properties PropertiesChanged (sa{sv}as) signals from the native InputPlumber server (new EmitStringProp/EmitArrayProp methods in tests/native_ip_server.c) and drives the client's real sd-bus process loop, asserting props_state.observed + value for ProfileName, ProfilePath, GamepadOrder, TargetDevices, SourceDevicePaths through the real parse path. Verified: ./scripts/verify.sh exit 0 (91/91, only runner-gated skips) and ctest --test-dir build -R 'test_properties_changed|test_overlay_native|test_manager_native|test_native_dbus' --output-on-failure exit 0 (5/5); new test_properties_changed_reactive_string/array pass.
