...
- Documentation impact: synchronize `FACTORY-LOOP-SPEC.md`, factory operations, README, state schema, conformance wording, and installed help with the ordered once-per-round semantics and intentionally disabled Ollama entry.
- Evidence: implemented a strict fixed-implementation registry (`branch_guard` enabled/mandatory, `ollama_usage_guard` disabled/mandatory), single-commit descriptor-anchored configuration binding, write-ahead start/completion cursors, canonical typed result chaining, atomic failed-result/terminal publication, and planner-retry suppression. Per-model launch has no cookie/quota controls and exposes no usage module; the foreign-state `show` surface was removed. Unit/state/launch/usage/campaign/adversarial/migration/installed suites pass, including exact five-round order/count, retry, forged-registry, postcondition, and crash-window regressions. The final serial Nix `verify-boilerplate.sh` gate passed. Independent final security review returned `SECURITY-ACCEPTABLE YES`. The broader Nix project gate remains findings: `test_installed_functional_evidence` reports an escaping override and the three known editor diagram golden comparisons fail; kernel-controller/backend hardware tests remain skipped. No golden or evidence tier was changed, and no foreign `.ralph` or `.factory-state` content was accessed.
