...
Runner: none
Evidence: repair cycle 2: the fail-closed readiness sequence is now defined once in overlay_wire_required_steps() (src/app/overlay_service.c) and shared by run_overlay_service() startup and overlay_recover(), removing the drifted duplicate that registered triggers via cbx_trigger_register_all() and called set_all_pass() unconditionally. Recovery retry is paced across the two-second window (CBX_RECOVERY_ATTEMPT_INTERVAL_MS / CBX_MANAGER_RECOVERY_ATTEMPT_INTERVAL_MS), refuses to reset an in-flight attempt budget, and degrades only on the transition, so a persistent DBus process error cannot churn the UI thread or flood logs; added native test_manager_recovery_attempts_are_paced. ./scripts/verify.sh exit 0 (91/91 ctest, 2 hardware skips); targeted ctest regex exit 0 (7/7); ./scripts/verify-sanitizers.sh under nix-shell exit 0 (91/91, no ASan/UBSan defects).
