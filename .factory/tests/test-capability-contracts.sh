...
must_fail "empty must_not_skip marker list" \
    "cd '$tmp/empty-skip-markers' && ./.factory/tools/check-capability-contracts.py"
...
runner_class_bad=0
for bad in 'GpuRunner' '' 'gpu runner' 'gpu/runner' 'G'; do
...
open(path, "w", encoding="utf-8").write(json.dumps(data, indent=2))
CONTRACT
(cd "$tmp/iprunner-candidate" && ./.factory/tools/check-capability-contracts.py >/dev/null)
...
must_fail "declared contract bound to an undeclared runner class" \
    "cd '$tmp/undeclared-runner-class' && ./.factory/tools/check-capability-contracts.py"
...
must_fail "required capability with no declared runner providing it" \
    "cd '$tmp/missing-required-capability' && ./.factory/tools/check-capability-contracts.py"
