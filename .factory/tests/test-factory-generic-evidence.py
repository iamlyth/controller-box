...
CHECKER = ROOT / ".factory/tools" / "check-installed-harness-evidence.sh"
STUB_SUITE = (
    "#!/usr/bin/env bash\n"
    "set -euo pipefail\n"
...
class GenericEvidenceSuite(unittest.TestCase):
    ...
    def _copy_surface(self, target: Path) -> None:
        ...
        # canonical spec at ``docs/SPEC.md`` from this fixture.
    ...
    def checker(self, check: bool = True):
        return _run(
            ["bash", str(self.fixture / ".factory/tools/check-installed-harness-evidence.sh")],
            cwd=str(self.fixture),
            env=self.base_env(),
            check=check,
        )
    ...
    def test_authority_path_change_after_evidence_is_stale(self) -> None:
        ...
        commits are not)."""
        self.publish_all()
        checker = self.fixture / ".factory/tools/check-installed-harness-evidence.sh"
        checker.write_text(
