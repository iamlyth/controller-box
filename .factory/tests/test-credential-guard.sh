...
expect_block coordinator-runner "coordinator-only-runner" './.factory/runner/run-factory-runners.py'
expect_block coordinator-runner-shell "coordinator-only-runner" './.factory/runner/run-factory-runners.py; true'
expect_block coordinator-runner-comment "coordinator-only-runner" './.factory/runner/run-factory-runners.py#comment'
expect_block coordinator-runner-python "coordinator-only-runner" 'python3 .factory/runner/run-factory-runners.py'
expect_block coordinator-runner-absolute "coordinator-only-runner" '/workspace/controller-box/.factory/runner/run-factory-runners.py'
expect_block coordinator-runner-sh-c "interpreter-bypass" 'sh -c ./.factory/runner/run-factory-runners.py'
expect_block coordinator-runner-single-quoted "coordinator-only-runner" "'./.factory/runner/run-factory-runners.py'"
expect_block coordinator-runner-double-quoted "coordinator-only-runner" '"./.factory/runner/run-factory-runners.py"'
...
# A value-taking sudo option (``-h host``) consumes its value so the inner
# command word is still inspected; an unknown option fails closed the same way
# (treated as value-taking) so it can never hide the inner verb behind a value.
expect_block sudo-host-ps "ps-environ-column" 'sudo -h host ps eww'
expect_block sudo-host-cat "procfs-environ-cmdline" 'sudo -h host cat /proc/self/environ'
expect_block sudo-unknown-opt "ps-environ-column" 'sudo -X foo ps eww'
expect_block sudo-unknown-opt-cat "procfs-environ-cmdline" 'sudo -X foo cat /proc/self/environ'
...
expect_allow sudo-flag-cmake 'sudo -n cmake --build build-check --parallel'
