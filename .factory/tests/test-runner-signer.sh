...
root=pathlib.Path(sys.argv[1]); p=root/'.factory/runner/factory-runner-signer.py'
...
install=(root/'.factory/runner/install-factory-runner-v2.sh').read_text()
assert 'old-signer-sudoers' in install and '/etc/sudoers.d/factory-runner-signer' in install
assert 'pwd.getpwuid' in install and 'factory-runner-v2.bundle' in install
assert 'trap rollback_signal EXIT INT TERM HUP' in install
assert 'FACTORY_BROKER_SIGNING' not in (root/'.factory/runner/factory-runner-signer.py').read_text()
