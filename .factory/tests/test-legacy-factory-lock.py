...
helper=$SCRIPT_DIR/.factory/tools/factory-lock-exec.py
# shellcheck source=.factory/tools/factory-lock.sh
source "$SCRIPT_DIR/.factory/tools/factory-lock.sh"
...
helper="$renamed/.factory/tools/factory-lock-exec.py"
...
mv "$renamed" "$root"
trap - EXIT
python3 "$root/.factory/tools/factory-lock-exec.py" "$root" --check
""",
            encoding="utf-8",
        )
        probe.chmod(0o755)
...
def test_untrusted_signal_is_forwarded_and_reaped() -> None:
    ...
    try:
        ...
        command = (
            f"source {root}/.factory/tools/factory-lock.sh; FACTORY_LOCK_ROOT={root}; "
