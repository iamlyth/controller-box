...
add_bug() {
    ./.factory/tools/bug-ledger.py add --title "$1" --severity high --reported 2026-08-06 \
...
./.factory/tools/bug-ledger.py validate >/dev/null
[[ $(./.factory/tools/bug-ledger.py list | wc -l) -eq 3 ]]
...
set +e
./.factory/tools/bug-ledger.py close BUG-0001 --resolution fixed --verification tested --closed 2026-08-07 >/dev/null 2>&1
not_in_progress_close=$?
set -e
[[ $not_in_progress_close -eq 1 ]]
./.factory/tools/bug-ledger.py set-status BUG-0001 in_progress
set +e
./.factory/tools/bug-ledger.py set-status BUG-0001 open >/dev/null 2>&1
bad_transition=$?
./.factory/tools/bug-ledger.py close BUG-0001 --resolution '' --verification evidence --closed 2026-08-07 >/dev/null 2>&1
bad_close=$?
set -e
[[ $bad_transition -eq 1 && $bad_close -eq 1 ]]
./.factory/tools/bug-ledger.py close BUG-0001 --resolution "Corrected parser" --verification "Regression test passed" --closed 2026-08-07
./.factory/tools/bug-ledger.py validate >/dev/null
./.factory/tools/bug-ledger.py show BUG-0001 | grep -q '"status": "closed"'

# Link/unlink are validated mutations and do not alter immutable fingerprints.
before=$(./.factory/tools/bug-ledger.py fingerprint BUG-0002)
./.factory/tools/bug-ledger.py set-status BUG-0002 triaged
after_status=$(./.factory/tools/bug-ledger.py fingerprint BUG-0002)
./.factory/tools/bug-ledger.py link BUG-0002 github https://github.com/example/project/issues/22
set +e
./.factory/tools/bug-ledger.py link BUG-0002 github https://github.com/example/project/issues/23 >/dev/null 2>&1
duplicate_provider_rc=$?
set -e
[[ $duplicate_provider_rc -eq 1 ]]
after_external=$(./.factory/tools/bug-ledger.py fingerprint BUG-0002)
./.factory/tools/bug-ledger.py unlink BUG-0002 github
...
PY
    set +e
    ./.factory/tools/bug-ledger.py validate >/dev/null 2>&1
    rc=$?
    set -e
...
[[ $(./.factory/tools/bug-ledger.py list | tail -1 | cut -f1) == BUG-10000 ]]

# Recover only the detectable destination-first interrupted-close duplicate.
./.factory/tools/bug-ledger.py set-status BUG-0003 triaged
./.factory/tools/bug-ledger.py set-status BUG-0003 planned
./.factory/tools/bug-ledger.py set-status BUG-0003 in_progress
cp .factory/bugs/open.md before-close.md
./.factory/tools/bug-ledger.py close BUG-0003 --resolution fixed --verification tested --closed 2026-08-07
python3 - <<'PY'
import json, re
...
PY
set +e; ./.factory/tools/bug-ledger.py validate >/dev/null 2>&1; interrupted_rc=$?; set -e
[[ $interrupted_rc -eq 1 ]]
./.factory/tools/bug-ledger.py recover >/dev/null
./.factory/tools/bug-ledger.py validate >/dev/null
...
printf '\ndirty contract\n' >> docs/SPEC.md
set +e
./.factory/tools/check-maintenance-freshness.sh >/dev/null 2>&1
dirty_spec_rc=$?
set -e
[[ $dirty_spec_rc -eq 1 ]]
...
./.factory/tools/bug-ledger.py set-status BUG-0001 planned
./.factory/tools/bug-ledger.py set-status BUG-0001 in_progress
./.factory/tools/bug-ledger.py close BUG-0001 --resolution fixed --verification tested --closed 2026-08-07
set +e
./.factory/tools/check-maintenance-freshness.sh >/dev/null 2>&1
premature_rc=$?
set -e
...
open(p, 'w', encoding='utf-8').write(text)
PY
    set +e; ./.factory/tools/validate-maintenance-plan.py complete .factory/artifacts/maintenance-plan.md >/dev/null 2>&1; rc=$?; set -e
...
open(p, 'w', encoding='utf-8').write(text)
PY
set +e; ./.factory/tools/check-maintenance-freshness.sh >/dev/null 2>&1; base_binding_rc=$?; set -e
