...
python3 "$ROOT/.factory/tests/test-runner-authority.py"
python3 "$ROOT/.factory/tests/test-runner-artifacts.py"
python3 "$ROOT/.factory/tests/test-broker-security.py"
python3 "$ROOT/.factory/tests/test-runner-client-security.py"
pycache=$(mktemp -d)
trap 'rm -rf -- "$pycache"' EXIT HUP INT TERM
PYTHONPYCACHEPREFIX="$pycache" python3 -m py_compile "$ROOT/.factory/runner/factory-runner-broker.py" "$ROOT/.factory/runner/factory-runner-server.py" "$ROOT/.factory/runner/factory_runner_authority.py" "$ROOT/.factory/runner/run-factory-runners.py" "$ROOT/scripts/check-factory-runner-evidence.py"
bash -n "$ROOT/.factory/runner/install-factory-runner-v2.sh" "$ROOT/deploy/factory-runner-authority-v1/probe-controller-production-routing.sh" "$ROOT/deploy/factory-runner-authority-v1/probe-gpu-compositor.sh"
