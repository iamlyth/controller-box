...
def make_repo_force(root: Path, files: dict, force: list[str]) -> None:
    """Like :func:`make_repo` but force-adds the listed ignored paths.

    Forbidden legacy pathnames under git-ignored namespaces (``.ralph/``)
    must be force-added so the planted path is genuinely tracked.
    """
    make_repo(root, files)
    for rel in force:
        _run(root, "add", "-f", "--", rel)
    _run(root, "commit", "-qm", "force-add forbidden paths")
...
class TestTrackedInventory(FootprintTestCase):
    ...
    def test_tracked_ralph_any_path_fails(self) -> None:
        # Post-removal: *any* tracked ``.ralph/**`` pathname is a migration
        # regression (the Ralph control plane was removed, so no legacy file
        # may be tracked at all).
        repo = self.repo(
        ...
        self.assertFalse(report.ok)
        self.assertTrue(
            any("legacy namespace is tracked (must be removed)" in error
                for error in report.errors)
        )
    ...
    def test_tracked_ralph_symlink_mode_fails(self) -> None:
        # A tracked ``.ralph/**`` entry fails regardless of its mode: the
        # post-removal contract forbids every tracked legacy pathname.
        root = self.root / "repo"
        root.mkdir()
        ...
        (root / ".pi" / "agents").mkdir(parents=True)
        (root / ".pi" / "agents" / "reviewer.md").write_text("---\n")
        (root / ".pi" / "subagents.json").write_text("{}")
        ...
        report = footprint.inventory(root)
        self.assertTrue(
            any("legacy namespace is tracked (must be removed)" in e
                and ".ralph" in e for e in report.errors)
        )
...
class TestOnDiskInventory(FootprintTestCase):
    ...
    def test_on_disk_ralph_root_symlink_fails(self) -> None:
        # The foreign ``.ralph/`` namespace root is lstat'ed only: a symlink
        # at the root is rejected (never followed), while the directory's
        # contents are never enumerated, opened, or read.
        repo = self.root / "repo"
        make_repo(repo, {**product_files(), **harness_files()})
        target = self.root / "outside-ralph"
        target.mkdir()
        os.symlink(target, repo / ".ralph")
        report = footprint.inventory(repo)
        self.assertFalse(report.ok)
        self.assertTrue(
            any("legacy namespace is a symlink" in error
                for error in report.errors)
        )

    def test_on_disk_ralph_root_fifo_fails(self) -> None:
        # A non-directory ``.ralph/`` root (a FIFO) is rejected; the root is
        # lstat'ed only and never followed.
        repo = self.root / "repo"
        make_repo(repo, {**product_files(), **harness_files()})
        os.mkfifo(repo / ".ralph")
        report = footprint.inventory(repo)
        self.assertFalse(report.ok)
        self.assertTrue(
            any("legacy namespace is not a directory" in error
                for error in report.errors)
        )

    def test_on_disk_ralph_foreign_content_allowed(self) -> None:
        # The foreign ``.ralph/`` directory may remain with arbitrary
        # content (ignored or not): the inventory never enumerates, opens,
        # or reads anything under it, so its contents are never judged.
        repo = self.root / "repo"
        make_repo(repo, {**product_files(), **harness_files()})
        ralph = repo / ".ralph"
        ralph.mkdir()
        ...
        (ralph / "agent").mkdir()
        (ralph / "agent" / "new.md").write_text("x")
        os.mkfifo(ralph / "pipe")
        report = footprint.inventory(repo)
        self.assertTrue(report.ok, "\n".join(report.errors))
