...
CHECKER="$PROJECT_ROOT/.factory/tools/check-golden-policy.py"
...
expect() {
    ...
    set +e
    (cd "$dir" && ./.factory/tools/check-golden-policy.py >/dev/null 2>&1)
    local rc=$?
    set -e
    [[ $rc -eq $expected ]] || {
...
(cd "$tmp/blessed" && GOLDEN_REVIEWER='env reviewer' \
    ./.factory/tools/check-golden-policy.py >/dev/null 2>&1)
env_reviewer_rc=$?
set -e
