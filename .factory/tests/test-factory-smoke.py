...
class SmokeWorkspace:
    ...
    def build(self) -> None:
        ...
        spec_blob = _git(
            ws, "rev-parse", "HEAD:docs/SPEC.md"
        ).stdout.strip()
        common_front = {
...
class EvidenceSmokeUnit(_SmokeBase):
    ...
    def _pre_smoke_plan() -> bytes:
        ...
        The canonical plan's pending set changes as the product advances, so
        the fixture derivation pins the evidence lane's shape explicitly:
        every runnable pending task before Task 22 (and Task 22 itself, when
        the live plan already completed it) is forced to ``pending``/``blocked``
        so the deterministic selector picks exactly the evidence task.  This is
        a fixture-only assumption about the derived pre-round plan; the
...
class EvidenceSmokeRound(_SmokeBase):
    ...
    def test_full_round_success_with_foreign_state_preserved(self) -> None:
        ...
        self.assertEqual(
            ...
            sha256(_git_blob(ws.root, final_head, "docs/SPEC.md")),
        )
        self.assertEqual(
