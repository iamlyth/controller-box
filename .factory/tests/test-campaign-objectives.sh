    ...
    cat > "$dir/.factory/capability-contracts.json" <<'CONTRACTS'
{
  ...
  "capabilities": [
    {
      "name": "runner-gate",
      "probe_argv": ["./.factory/runner/run-factory-runners.py"],
      "probe_marker": "",
      "must_execute": true,
      "must_not_skip": [],
    ...
    cat > "$dir/.factory/campaign-objectives.json" <<'OBJECTIVES'
{
  ...
  "objectives": [
    ...
    {"key": "real-system", "label": "real", "receipt_categories": ["real-system-service", "target-consumer", "controller-production-routing"]}
  ]
}
OBJECTIVES
    cat > "$dir/.factory/campaign-receipt-policy.json" <<'POLICY'
{
  ...
  "categories": [
    {"name": "runner-evidence", "tier": "installed", "allow_manifest": true, "argv": [["./.factory/runner/run-factory-runners.py"]]},
    ...
    {"name": "controller-production-routing", "tier": "real_system", "allow_manifest": true, "argv": [["./scripts/probe-routing.sh"]]}
  ]
}
POLICY
...
write_evidence() {
    ...
    mkdir -p "$dir/.factory-state/runner-evidence/synthetic-campaign-objectives/${FACTORY_READINESS_NONCE}/$runner/$head/$zero"
    local public_key
...
manifest = {
    "schema": "factory-runner-receipt/v3", "result": "pass", "runner": runner,
    "commit": head, "tree": tree, "environment_blob": environment_blob,
    "archive_sha256": archive_sha256,
    "campaign_id": "synthetic-campaign-objectives", "readiness_nonce": "2" * 64, "authority_sha256": "a" * 64, "nonce": "0" * 64,
...
manifest_path = root / f".factory-state/runner-evidence/synthetic-campaign-objectives/{'2'*64}/{runner}/{head}/{'0'*64}/manifest.json"
manifest_path.write_bytes(raw)
aggregate_path = root / f".factory-state/runner-evidence/synthetic-campaign-objectives/{'2'*64}/aggregate.json"
aggregate_path.parent.mkdir(parents=True,exist_ok=True)
if aggregate_path.exists():
    aggregate = json.loads(aggregate_path.read_text())
else:
    aggregate = {"schema": "factory-runner-aggregate/v4", "campaign_id": "synthetic-campaign-objectives", "readiness_nonce": "2" * 64, "commit": head,
                 "tree": tree, "environment_blob": environment_blob, "runners": []}
aggregate["runners"] = [
    item for item in aggregate["runners"] if item["name"] != runner
] + [{"name": runner, "manifest": f".factory-state/runner-evidence/synthetic-campaign-objectives/{'2'*64}/{runner}/{head}/{'0'*64}/manifest.json",
...
PY
    cat "$dir/.factory-state/runner-evidence/synthetic-campaign-objectives/${FACTORY_READINESS_NONCE}/$runner/$head/$zero/manifest.json" \
        | ssh-keygen -Y sign -f "$tmp/signer-key-$runner" -n factory-runner-receipt \
            > "$dir/.factory-state/runner-evidence/synthetic-campaign-objectives/${FACTORY_READINESS_NONCE}/$runner/$head/$zero/manifest.sig" 2>/dev/null
    python3 - "$dir/.factory-state/runner-evidence/synthetic-campaign-objectives/${FACTORY_READINESS_NONCE}/$runner/$head/$zero/manifest.sig" \
        "$dir/.factory-state/runner-evidence/synthetic-campaign-objectives/${FACTORY_READINESS_NONCE}/aggregate.json" "$runner" <<'PY'
...
- Executable evidence: ./.factory/runner/run-factory-runners.py PASS [receipt: .factory-state/audit-receipts/runner-evidence.json]
...
receipt "$tmp/blessed" 1 runner-evidence 0 ./.factory/runner/run-factory-runners.py
...
receipt "$tmp/blessed" 3 controller-production-routing 0 ./scripts/probe-routing.sh
...
- Executable evidence: ./scripts/probe-routing.sh PASS [receipt: .factory-state/audit-receipts/controller-production-routing.json]
...
# category (runner-evidence) at the audit base: the fake-runner manifest's
# recorded capabilities contain runner-gate, whose committed contract probe
# argv equals the category's allowlisted argv exactly.
receipt "$tmp/blessed" 1 project-verify 0 ./scripts/verify-project.sh
cat > "$tmp/blessed/.factory/artifacts/campaign-audit.md" <<REPORT
---
result: pass
...
## Evidence reviewed
- Executable evidence: ./scripts/verify-project.sh PASS [receipt: .factory-state/audit-receipts/project-verify.json]
- Executable evidence: ./scripts/probe.sh PASS [manifest: .factory-state/runner-evidence/synthetic-campaign-objectives/${FACTORY_READINESS_NONCE}/fake-runner/$head/$(printf '0%.0s' {1..64})/manifest.json]
...
## Evidence reviewed
- Executable evidence: ./scripts/verify-project.sh PASS [receipt: .factory-state/audit-receipts/project-verify.json]
- Executable evidence: ./scripts/probe.sh PASS [manifest: .factory-state/runner-evidence/synthetic-campaign-objectives/${FACTORY_READINESS_NONCE}/real-system-service/$head/$(printf '0%.0s' {1..64})/manifest.json]
...
# Campaign Audit
## Evidence reviewed
- Executable evidence: ./scripts/probe.sh PASS [manifest: .factory-state/runner-evidence/synthetic-campaign-objectives/${FACTORY_READINESS_NONCE}/verify-runner/$head/$(printf '0%.0s' {1..64})/manifest.json]
