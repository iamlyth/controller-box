...
[
  {
    ...
    "actual": "The original production observation remains unevidenced as resolved: 0 of 4 virtual controllers active and no usable routed controller. The bounded product remediation now treats configured virtual slots independently of physical composites (including four ready targets with zero composites), retains CreateTargetDevice paths, confirms exact publication/type/removal with monotonic deadlines, attaches only persisted PersistentId assignments, requires exact singleton TargetDevices sets, reports categorized phase/count/deadline/cleanup diagnostics, and refreshes the visible Manager Controllers tab with a ready-but-unassigned state. Manager mutations use exact-path bounded confirmation and cleanup instead of one-refresh count/index assumptions. The production-routing probe no longer calls AttachTargetDevice directly and rejects ambiguous independently sorted identical-name node pairing. The latest controller remediation also synchronizes the durable manager selection with the real list widget across keyboard/controller/pointer/focus/refresh paths, uses authoritative exact `TargetDevices` replacement for assignment transfer and clearing, enforces the target limit before create, and compensates settings-write failures. Private native-signature sd-bus and synthetic adversarial tests remain supplemental software evidence. They do not prove the real InputPlumber system bus, kernel targets, or physical routing.",
  ...
  },
  {
    ...
    "actual": "The factory documents Pi as the production backend but its file-granular Landlock policy is incompatible with the jailed Pi2 bootstrap. Broad Nix-store execution or exposing the host agent directory would weaken existing security boundaries, so experimental provider/closure changes were reverted rather than committed as a bypass.\n\nCandidate fix (uncommitted working-tree surface): authenticated node/cli identities are threaded into the exact-inode confinement; the backend runtime closure is read-only and only the anchored canonical `/dev/null` receives a discard sink (the single anchored device write grant); no directory-level EXECUTE, host agent directory, or network/env widening is added. Validation in the working tree: confinement suite 104/104, usage suite 113/113 with one declared environmental skip, launch suite 108/108, and the complete `verify-boilerplate.sh` gate passes, with an independent security verdict BUG0019-FIX-SECURE YES.",
