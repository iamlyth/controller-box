...
[
  ...
  },
  {
    ...
    "resolution": "Consolidated lifecycle state I/O and locking; moved the retained lock into owner-private Git state with safe legacy migration; dropped lock descriptors and metadata before untrusted Ralph/Pi execution; bound stale loop-lock removal to inode, boot, and process start identity; added no-follow atomic markers, receiver-side lifecycle-token contamination rejection, phase launch handshakes, durable one-time supervision/verifier migration, strict future-field and maintenance-dependency validation, and committed-verifier blob binding.",
    "verification": "Changed-script bash/Python/Node/mode/diff checks passed; focused factory-lock, orchestration-security, campaign-state/campaign, completion/stale/recovery, plan-cycle, bug-workflow, checkpoint, Pi-wrapper, and sentinel tests passed; complete factory test set and ./scripts/verify-boilerplate.sh passed; nix-shell --run ./scripts/verify-project.sh passed 98/98 CTest targets (one declared backend skip), mandatory installed-functional acceptance, packaging, and installed smoke. Campaign JSON remained SHA-256 800ced3fd2c6889913d1035906fdc9093d76c0ad2ebe4162c1c380b547561b91.",
    "closed": "2026-08-19"
  }
]
```
