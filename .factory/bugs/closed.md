...
[
  ...
  },
  {
    ...
    "actual": "The initial hardening left inherited-lock, lock-path, stale-owner, marker, completion-receiver, launch-handshake, migration, dependency, and verifier-binding gaps identified by independent review.",
    ...
    "resolution": "Replaced replaceable lock-file authority with Linux flock on the canonical repository-root directory FD; dropped every root-inode descriptor and lock variable at implementation-controlled executable boundaries; safely locked and quarantined legacy .factory-lock migration; bound verification to immediately recomputed secure tracked config/executable blobs and canonical argv; bound launch confirmation to exact whole-stream/delta/start-record bytes, sizes, digests, and nonces; made completion/state/Ralph-lock removals retained-FD dirfd/no-follow quarantine operations; enforced strict receiver-side event protocol/token contamination, secure modes, required Linux primitives, and strict maintenance field ordering.",
    "verification": "Python compile, bash -n, ShellCheck, git diff --check, mode checks, and bug-ledger validation passed. Focused test-factory-lock.py, test-orchestration-security.py, test-ralph-campaign-state.py, test-ralph-campaign.sh, test-ralph-completion-recovery.sh, test-plan-cycle.sh, test-ralph-stale-recovery.sh, test-ralph-recover-safety.sh, test-git-checkpoint.sh, test-bug-workflow.sh, test-campaign-audit.sh, test-scratchpad-guard.sh, test-installed-functional-evidence.sh, test-factory-environment.sh, test-factory-runner.sh, test-pi2-ollama-wrapper.sh, test-production-path-bypass.sh, and test-boilerplate.sh passed serially. ./scripts/verify-boilerplate.sh passed. nix-shell --run ./scripts/verify-project.sh passed 98/98 CTest targets with two declared environment skips (kernel controller and GPU backend), mandatory installed-functional acceptance, packaging, and installed smoke. Campaign JSON remained SHA-256 800ced3fd2c6889913d1035906fdc9093d76c0ad2ebe4162c1c380b547561b91.",
    "closed": "2026-08-19"
  }
]
```
