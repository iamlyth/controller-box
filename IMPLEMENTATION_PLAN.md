...
| REQ-023 | §5.7 Manager interaction acceptance | partial | Interaction inventory exists (`tests/interaction_inventory.h/.c` with M01-M38, O01-O12, D01-D08); `test_manager_interaction_ctrl.c` (28 sub-tests) exercises both controller-path and pointer-path SDL events through `cbx_manager_handle_event` for M01-M09 (Controllers) and M21-M27 (Settings) with semantic outcome assertions; D01, D02, D05, D06 disabled scenarios tested through both paths; M10-M20 (Profiles) and M28-M38 (Editor) pending in Task 9; installed smoke test still needs coordinate-based mouse clicks (Task 12) | Task 2, Task 3, Task 7, Task 8, Task 9, Task 12 |
