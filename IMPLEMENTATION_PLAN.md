...
| REQ-007 | §4.1 Select-screen model | verified | Grid render (`grid_render.c`) and layout match spec; multi-controller input wired into overlay production poll loop via `cbx_overlay_input_build_map` and `cbx_overlay_input_cb` dispatching to `cbx_player_mode_handle` with correct row index (REQ-009); `test_overlay_interaction.c` O01–O12 verify through `cbx_overlay_service_step` | — |
...
| REQ-010 | §4.4 Host Mode (R3, exclusive) | verified | `host_mode.c` implements enter/exit/navigate/freeze/slot via production dispatch; overlay service dispatches both keyboard and DBus InputEvent signals to host mode via `cbx_overlay_input_cb` with the host_row; production-poll-loop verified in `test_overlay_interaction.c` (O06–O09, O11b: enter/navigate/slot/exit/freeze via keyboard and DBus InputEvent through `cbx_overlay_service_step`). Host-mode profile cycling (host "editing slot/profile") is deferred per §13: "Host Mode interface details — how the host navigates between rows, what editing looks like. The mode and its trigger (R3) are specified; the interior UX is specified during implementation." UP/DOWN navigates rows per the current implementation; profile cycling awaits implementation of the deferred interior UX. | — |
...
| REQ-023 | §5.7 Manager interaction acceptance | verified | Interaction inventory exists (`tests/interaction_inventory.h/.c` with M01-M38, O01-O12, D01-D08); `test_manager_interaction_ctrl.c` (28 sub-tests) exercises M01-M09 (Controllers) and M21-M27 (Settings) via both controller and pointer paths through `cbx_manager_handle_event`; `test_manager_interaction_prof.c` (35 sub-tests) exercises M10-M20 (Profiles) and M28-M38 (Profile editor) via both paths through `cbx_manager_handle_event`; D01, D02, D05, D06, D03, D04, D07, D08 disabled scenarios tested; O01-O12 (overlay) verified in `test_overlay_interaction.c` (19 sub-tests) through `cbx_overlay_service_step`; installed smoke test (`tests/test_installed_smoke.sh`) performs coordinate-based mouse clicks on body controls (profiles tab, settings tab, settings list item, Save button) with screenshot diff and file mutation verification | — |
...
| REQ-029 | §11.1 Rendering verification (7 layers) | verified | Layers 1–4 and 6 verified (framebuffer tests, golden images, backend smoke, failure artifacts); layer 5 (installed smoke) verified for keyboard input AND coordinate-based mouse clicks on body controls (profiles tab, settings tab, settings list item, Save button via xdotool + screenshot diff + file mutation); layer 7 (human release acceptance) is a pre-promotion gate documented in OPERATIONS.md, not an autonomous-cycle verification | — |
| REQ-030 | §11.2 Autonomous definition of done | verified | All 9 items met: (1) conformance matrix — every REQ-001–REQ-032 verified; (2) production-path behavior — all acceptance tests use `cbx_manager_handle_event` or `cbx_overlay_service_step`, no direct-call-only sole evidence; (3) interaction traversal — M01-M38, O01-O12, D01-D08 all tested with semantic outcome verification; (4) visual/degraded-state — framebuffer tests and golden baselines pass for normal, empty, loading, unavailable, validation-error, backend-error, and recovery states; (5) regression/quality — 78/78 tests pass (1 skip: backend_smoke), `verify-project.sh` + `verify-boilerplate.sh` green, no compiler warnings; (6) known-defect accounting — `open-bugs.md` empty `[]`, BUG-0001/0002/0003 all closed with verification; (7) independent review — parallel correctness, security, and documentation reviews launched; (8) documentation — README.md and OPERATIONS.md match observed behavior; (9) repository integrity — Git tree clean on `develop`, plan status `complete` | — |
...
- Verification: `nix-shell --run "./scripts/verify-project.sh"` — passes (includes installed smoke test with coordinate-based mouse clicks). **Production changes**: added Step 3b to `tests/test_installed_smoke.sh` with four coordinate-based xdotool mouse clicks: (1) Profiles tab at (639,24) — screenshot diff verifies visible state change; (2) Settings tab at (1065,24) — screenshot diff; (3) Settings list item at (100,90) — screenshot diff; (4) Save button at (116,522) — verifies `settings.yaml` file mutation (mtime/creation). Image comparison via ImageMagick `convert -compose difference`; body region cropped to y=48..720. Existing keyboard input, non-blank verification, skip-77 behavior, and real binary launch all preserved. `docs/OPERATIONS.md` updated with click targets and expected output.
- Documentation impact: `docs/OPERATIONS.md` — installed smoke test description updated with coordinate-based click interactions.
