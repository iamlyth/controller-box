...
| M37 | Save and close editor | B (or A on close) from list mode | Mouse click on close area | Profile written to disk via `cbx_profile_save_to_dir` with NES validation; editor closes; profile list refreshes | `cbx_manager_handle_event` → `cbx_profile_save_to_dir` → editor close | Task 4, Task 5, Task 9 |
...
- Acceptance criteria:
  - **Widget A-key handling**: `widget_button.c` and `widget_list.c` `handle_event` handle `SDLK_a` with the same press/release semantics as `SDLK_RETURN`/`SDLK_SPACE` (keydown sets pressed/selected, keyup fires callback). This is required because the manager's `handle_event` tries the focused widget first; if the widget doesn't handle `SDLK_a`, the event falls through to the manager which currently swallows it.
  - **Tab-specific activation forwarding**: `cbx_manager_handle_event` forwards `SDLK_RETURN`/`SDLK_SPACE`/`SDLK_a` (A button) keyup to the active tab's activate function: `cbx_controllers_tab_activate()`, `cbx_profiles_tab_activate()`, or `cbx_settings_tab_activate()` — not just returning `true`. The tab activate function checks the tab's current mode and dispatches appropriately (e.g. type picker confirm, name input confirm, delete confirm, settings activate/edit).
  - **Controllers tab**: type picker list `on_select` callback is set so confirming a type calls `cbx_controllers_tab_confirm_type_pick()` through the event path (currently NULL — only called from tests). B while type picker is open cancels the picker.
  - **Profiles tab — create source picker**: Create button opens the create source picker (Default copy / Empty / Clone) instead of hardcoding `CBX_PT_CREATE_DEFAULT_COPY`. The picker list is populated with the three options. Selecting a source enters name-input mode. The create picker's `on_select` is wired.
  ...
  - **Settings tab**: A on a settings list item calls `cbx_settings_tab_activate()`; Up/Down while in edit mode call `cbx_settings_tab_edit_up()`/`cbx_settings_tab_edit_down()`; A while in edit mode calls `cbx_settings_tab_confirm_edit()`; B while in edit mode calls `cbx_settings_tab_cancel_edit()`.
...
- Acceptance criteria:
  ...
  - `cbx_profile_save_to_dir()` uses the canonicalized path from `realpath()` for the actual file write, not just for the validation check — eliminating the TOCTOU race between check and write (security defense-in-depth).
