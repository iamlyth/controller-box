...
if [[ -n "$FIXTURE" ]]; then
    ...
    if head_resolved=$(git -C "$PRODUCT_ROOT" rev-parse HEAD 2>/dev/null) && \
...
# ---------------------------------------------------------------------------
if [[ -z "$ARTIFACTS" ]]; then
    fail broker-artifact-dir-missing "root broker did not provide an artifact export directory"
fi
mkdir -p "$ARTIFACTS"
: > "$ARTIFACTS/probe.log"
log() { # msg
...
# must win.
head_commit=$(git -C "$PRODUCT_ROOT" rev-parse HEAD 2>/dev/null || true)
head_tree=$(git -C "$PRODUCT_ROOT" rev-parse 'HEAD^{tree}' 2>/dev/null || true)
[[ "$head_commit" =~ ^[0-9a-f]{40}$ && "$head_tree" =~ ^[0-9a-f]{40}$ ]] || fail exact-commit-unresolved "cannot resolve HEAD in broker product checkout"
mkdir -p "$tmp/source"
if ! git -C "$PRODUCT_ROOT" archive "$head_commit" | tar -x -C "$tmp/source"; then
