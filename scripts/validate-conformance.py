...
Rules:
- requirement IDs must agree exactly across the committed requirement-policy
  map (`.factory/requirement-policy.json`, the normative authority for the
  Controller's stable ID set), the flat §24 registry
  (`.factory/schemas/factory-plan-v1.requirements.json`, which must mirror the
  policy ID set in policy order), the plan matrix, and the sidecar: a missing,
  extra, renumbered, or duplicated ID anywhere fails closed (no ID may
  silently fall out of one authority while staying in another), and every
  committed JSON load uses an object-pairs hook that rejects duplicate object
  keys;
...
- `required_tier` is assigned by the separately committed requirement-policy
  map, never self-declared by the sidecar; the sidecar must match it exactly.
  When the policy entry declares `required_capabilities`, the sidecar must
  match that set exactly too (a capability relaxation is rejected); a policy
  entry without the key leaves the sidecar's capabilities self-declared (they
  are still validated for real-system/human tiers and capability evidence);
...
def plan_matrix(text: str) -> dict[str, str] | None:
    ...
    for line in match.group(1).splitlines():
        ...
        if requirement_id in rows:
            fail(f"plan matrix has a duplicate requirement ID: {requirement_id}")
...
def load_registry(root: Path) -> set[str]:
    """Load the committed requirement-ID registry, bound to the policy map.

    The requirement-policy map (`.factory/requirement-policy.json`) is the
    normative authority for the Controller's stable requirement-ID set
    (PLAN-01, Task 18 item 3): the plan matrix, the conformance sidecar, and
    the flat registry (`.factory/schemas/factory-plan-v1.requirements.json`)
    must each carry exactly these IDs, in policy order. A missing, malformed,
    or divergent registry fails closed so a requirement can never silently
    drop out of one file while staying in another.
    """
    ...
    if list(ids) != policy_ids:
        fail(
            "the requirement registry must contain exactly the "
            "requirement-policy IDs, in policy order"
        )
    return registry
...
def validate_ref_safety(ref: str, where: str) -> None:
    ...
    # The runtime namespaces are the only `.factory-state` refs allowed, and
    # only in `receipts`: the exact safe audit-receipts shape (no traversal,
    # no symlink, no nested path) or the exact runner-evidence manifest shape.
    if parts[0] == ".factory-state":
        if where.endswith(".receipts"):
            ...
            if remainder.startswith("audit-receipts/") and RUNTIME_RECEIPT_TAG.fullmatch(
                ...
                return
            if RUNTIME_MANIFEST_RE.fullmatch(remainder):
                return
        fail(
...
def runtime_ref_kind(ref: str) -> str | None:
    """Classify one exact live runtime receipt shape.

    Audit receipts and signed runner manifests have distinct authorities and
    must never be dispatched by their common ``.factory-state`` prefix.  The
    caller has already run ``validate_ref_safety``; repeat the exact shape
    classification here so a future caller cannot route a runner manifest to
    the audit-receipt parser (or vice versa).
    """
    if ref.startswith(RUNTIME_RECEIPT_PREFIX):
        tag = ref[len(RUNTIME_RECEIPT_PREFIX):]
        if RUNTIME_RECEIPT_TAG.fullmatch(tag):
            return "audit-receipt"
    prefix = ".factory-state/"
    if ref.startswith(prefix) and RUNTIME_MANIFEST_RE.fullmatch(ref[len(prefix):]):
...
def validate_runtime_receipt(root: Path, ref: str, evidence_commit: str) -> None:
    ...
    try:
        ...
        if state["base_commit"] != evidence_commit:
            fail(
                f"runtime receipt {ref} active coordinator base commit does "
                "not equal the row evidence_commit"
            )
        receipt = evidence.validate_receipt(
            root,
            ref,
...
def validate_runtime_manifest(root: Path, ref: str, evidence_commit: str) -> None:
    """Dispatch a live runner manifest to the signed runner authority.

    Runner evidence is intentionally ignored runtime state, not an audit
    receipt and not a Git blob.  Acceptance requires exact aggregate
    membership plus signature, commit/tree/environment/archive/argv/log
    bindings from ``check-factory-runner-evidence.py --verify-manifest``.
    """
    checker = root / "scripts/check-factory-runner-evidence.py"
    if checker.is_symlink() or not checker.is_file():
        fail(f"runner-evidence checker is unavailable: {checker}")
    try:
        result = subprocess.run(
            [
                sys.executable, str(checker), "--verify-manifest", ref,
                "--expected-commit", evidence_commit,
            ],
            cwd=root, text=True, capture_output=True, timeout=120,
        )
    except (OSError, subprocess.TimeoutExpired) as exc:
        fail(f"runtime runner manifest {ref} could not be validated: {type(exc).__name__}")
    if result.returncode != 0:
        detail = (result.stderr or result.stdout).strip()
        fail(
            f"runtime runner manifest {ref} is not accepted signed exact-commit "
            f"evidence: {detail or 'runner-evidence validation failed'}"
        )
...
def validate_verified_claim(root: Path, requirement: dict) -> None:
    ...
    for ref in refs:
        runtime_kind = runtime_ref_kind(ref)
        if runtime_kind == "audit-receipt":
            # Coordinator-minted audit receipts use the hidden no-follow
            # exact-mode authority and are never runner manifests.
            validate_runtime_receipt(root, ref, commit)
            continue
        if runtime_kind == "runner-manifest":
            # Signed runner manifests are live ignored evidence.  Route them
            # to the strong aggregate/signature checker, never the audit
            # receipt validator and never a Git-blob lookup.
            validate_runtime_manifest(root, ref, commit)
            continue
        if ref.startswith(".factory-state/"):
            fail(f"requirement {requirement_id} has an unclassified runtime ref: {ref}")
...
def cross_check_policy(data: dict, policy: dict[str, dict]) -> None:
    """Bind required_tier (and policy-declared capabilities) to the policy map."""
    ...
    for requirement in data["requirements"]:
        ...
        policy_capabilities = policy[requirement_id]["required_capabilities"]
        if policy_capabilities is not None and sorted(requirement["required_capabilities"]) != sorted(policy_capabilities):
            fail(
...
def cross_check_registry(data: dict, matrix: dict[str, str], policy: dict[str, dict], registry: set[str]) -> None:
    """Exact-set binding: sidecar == plan matrix == policy == registry."""
