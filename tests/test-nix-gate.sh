#!/usr/bin/env bash
# Regression for the authenticated Nix gate (scripts/nix-gate.sh,
# scripts/nix-gate-exec.sh, scripts/nix-gate-check.py).
...
for f in scripts/verify-project.sh scripts/verify-sanitizers.sh tests/test-visual-audit.sh; do
...
run_gate() { # <mode> ...(env vars already set in the subprocess env)
    ...
    env -u CBX_NIX_GATE_FILE -u CBX_NIX_GATE_NONCE -u IN_NIX_SHELL \
        bash -c 'source "$1/scripts/nix-gate.sh"; nix_gate_require "$2"' \
        _ "$PROJECT_ROOT" "$mode"
}
...
expect_rc 2 "forged nonce content is refused" \
    ...
    bash -c 'source "$1/scripts/nix-gate.sh"; nix_gate_require optional' _ "$PROJECT_ROOT"
...
if ! (PATH="$NO_NIX_PATH" command -v nix-shell >/dev/null 2>&1); then
    expect_rc 2 "wrapper fails when nix-shell is unavailable" \
        ...
        "$(command -v bash)" scripts/nix-gate-exec.sh /bin/true
else
...
INNER
    ...
    expect_rc 0 "authenticated wrapper round-trip executes the inner script" \
        bash scripts/nix-gate-exec.sh "$tmp/inner.sh"
