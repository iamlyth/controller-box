...
# A verifier failure must propagate through the real endpoint protocol and
# never produce a signed receipt (the endpoint refuses to sign failures).
...
sed -i 's/exit 23/exit 0/' "$tmp/repo/scripts/verify-project.sh"
git -C "$tmp/repo" add scripts/verify-project.sh
